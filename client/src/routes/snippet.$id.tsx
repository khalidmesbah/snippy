...
function SnippetDetail() {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        try {
          ...
          showNotification.info("Snippet access", "Trying public endpoint for snippet access");
        } catch (error) {
          showNotification.info("Snippet access", "Authenticated access failed, trying public endpoint");
        }
    ...
    select: (s: any) => ({ ...s, language: s?.language || "plaintext" }),
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) {
        showNotification.info("Delete response", `Delete response status: ${res.status}`);
  ...
  const forkMutation = useMutation({
    mutationFn: async () => {
      const res = await apiClient.post(`/snippets/${id}/fork`, { user_id: user?.id });
    ...
    onError: (error) => {
      showNotification.error("Failed to fork snippet", error instanceof Error ? error.message : "An error occurred");
    },
  });
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      showNotification.error("Copy failed", err instanceof Error ? err.message : "Failed to copy to clipboard");
    }
  };
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      showNotification.error("Share failed", err instanceof Error ? err.message : "Failed to copy URL to clipboard");
    }
  };
  ...
  if (isError || !snippet)
    return (
      <ErrorState
        message={(error as any)?.message || "Failed to load snippet"}
        onRetry={() => queryClient.invalidateQueries({ queryKey: ["snippets", "detail", id] })}
      />
    );
              ...
              <div className="flex flex-wrap gap-2 max-h-[100px] overflow-auto">
                {Array.isArray(snippet.tag_names) && snippet.tag_names.length > 0 ? (
                  snippet.tag_names.map((tagName) => (
                    <Badge key={tagName} variant="secondary" className="whitespace-nowrap">
                      {tagName}
                    </Badge>
                  ))
                ) : Array.isArray(snippet.tag_ids) && snippet.tag_ids.length > 0 ? (
                  snippet.tag_ids.map((tagId) => (
                    <Badge key={tagId} variant="secondary" className="whitespace-nowrap">
                      {tagId}
                    </Badge>
                  ))
                ) : null}
