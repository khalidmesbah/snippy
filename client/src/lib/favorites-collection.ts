 ...
 */
export function isFavoritesCollection(collection: Collection | FavoritesCollection): boolean {
 ...
 */
export function getFavoritedSnippetsCount(snippets: Array<{ is_favorite: boolean }>): number {
 ...
 */
export function getFavoritedSnippets<T extends { is_favorite: boolean }>(snippets: T[]): T[] {
...
export function sortCollectionsWithFavorites(
  ...
  const regularCollections = collections.filter(c => !isFavoritesCollection(c));

  // Sort regular collections by position or creation date
  let sortedRegularCollections = [...regularCollections];
  ...
  return favorites ? [favorites, ...sortedRegularCollections] : sortedRegularCollections;
}
