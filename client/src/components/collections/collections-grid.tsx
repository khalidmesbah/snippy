...
export const CollectionsGrid = forwardRef<{ handleSavePositions: () => void }, CollectionsGridProps>(({ 
  collections, 
  isLoading, 
  error, 
  onRefresh,
  ...
  const [items, setItems] = useState<(Collection | FavoritesCollection)[]>(collections);
  const [activeId, setActiveId] = useState<string | null>(null);
  const [hasPendingChanges, setHasPendingChanges] = useState(false);
  const [originalOrder, setOriginalOrder] = useState<(Collection | FavoritesCollection)[]>(collections);
  ...
  useImperativeHandle(ref, () => ({
    handleSavePositions: () => {
      if (onSavePositions) {
        const positions = items.map((item: Collection | FavoritesCollection, index: number) => ({
          id: item.id,
          position: index,
        }));
  ...
  // Handle collection deletion and update positions
  const handleDeleteSuccess = () => {
    // When a collection is deleted, we need to update positions for remaining collections
    // But don't call onPositionsUpdate here - wait for save button
    // Call the parent's onDeleteSuccess callback
    onDeleteSuccess?.();
  };
